...
class CNNClassifier(nn.Module):
    def __init__(self, sequence_length):
        ...
        conv_output_length = sequence_length // 4  # Two max-poolings (//2 twice)
        self.fc = nn.Linear(64 * conv_output_length, 1)

    def forward(self, x):
        x = x.view(x.size(0), -1, 4)       # (batch, seq_len, 4)
        x = x.permute(0, 2, 1)             # (batch, 4, seq_len) for Conv1d
        x = F.relu(self.conv1(x))         # (batch, 32, seq_len)
        x = self.pool1(x)                 # (batch, 32, seq_len/2)
        x = F.relu(self.conv2(x))         # (batch, 64, seq_len/2)
        x = self.pool2(x)                 # (batch, 64, seq_len/4)
...
def main():
    ...
    model = CNNClassifier(SEQUENCE_LENGTH)
