...
def main(input: str, output: str, exp_factor: int = 8, layer_idx: int = 23, sae_dir: str = None):
    ...
    for sequence in tqdm(seq_token_feats, desc="Building knowledge graph"):
        ...
        # gather and format annotation data
        seqmetadata = {}
        ...
        # add sequence node with annotations
        G.add_node(sequence, type="sequence", chrom=chrom, start=start, end=end, strand=strand, annotations=seqmetadata)
        ...
        for token, feat in seq_token_feats[sequence].items():
            ...
            G.add_edge(sequence, feat, token=token)

    # show graph info
    print(G)

    # save the knowledge graph as a json file
    print(f"Saving knowledge graph to {output}...")
